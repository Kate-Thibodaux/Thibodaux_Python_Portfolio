# Thibodaux_Python_Portfolio
This is the portfolio of python code that I learned during 450C.
...
def fahr_to_celsius(temp):
    # Return converted values more effeciently using the return function without a new variable. This code does the same thing as the previous function but it is more explicit in explaining how the return command works
...
```python
def offset_mean(data, targete_mean_value):
    """Return a new array containing the original data with its mean offset to match the desired value"""
    return(data - numpy.mean(data)) + target_mean_value
```
...
```
    ...
    offset_mean(data, targete_mean_value)
        Return a new array containing the original data with its mean offset to match the desired value
...
def normalize_rectangle(rect):
    ...
    x0, y0, x1, y1 = rect
    assert x0 < x1, 'Invalid X coorinates'
...
```python
# Promp the user to enter the input fasta file name
...
```python
# Promp the user to enter the output file name
